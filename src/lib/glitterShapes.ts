...
// and drawn as light, not as paint: layers accumulate with 'lighter' within
// one item, so the core clips to white while the bloom carries the colour.
// The overlay canvas itself composites normally — see glitter.ts for why a
// screen blend was tried and rejected.
//
// Build shapes from area, not from hairlines. A stroke covers almost no
// pixels, so however bright it is there is not enough of it to register
// against a photograph; six line-based shapes were cut for exactly that.
...
const drawSpark: Draw = (ctx, item, rand) => {
  bloom(ctx, 0.92, item.color, 0.62)
  const jitter = () => 0.85 + rand() * 0.3
  for (let i = 0; i < 4; i++) {
    chromaticSpike(ctx, (i / 4) * TAU, 0.98 * jitter(), 0.1, item.color, 0.98)
  }
  for (let i = 0; i < 4; i++) {
    spike(ctx, (i / 4) * TAU + Math.PI / 4, 0.36 * jitter(), 0.06, item.color, 0.6)
  }
  core(ctx, 0.2)
}
...
const drawTwinkle: Draw = (ctx, item, rand) => {
  bloom(ctx, 0.85, item.color, 0.58)
  for (let i = 0; i < 6; i++) {
    spike(ctx, (i / 6) * TAU, 0.95 * (0.72 + rand() * 0.42), 0.08, item.color, 0.95)
  }
  core(ctx, 0.17)
}
...
const drawBurst: Draw = (ctx, item) => {
  ...
  chromaticSpike(ctx, -Math.PI / 2, 0.99, 0.09, item.color, 1)
  chromaticSpike(ctx, Math.PI / 2, 0.99, 0.09, item.color, 1)
  spike(ctx, 0, 0.54, 0.075, item.color, 0.85)
  spike(ctx, Math.PI, 0.54, 0.075, item.color, 0.85)
...
const drawBokeh: Draw = (ctx, item) => {
  ...
  g.addColorStop(0, withAlpha(item.color, 0.2))
  g.addColorStop(0.55, withAlpha(item.color, 0.26))
  g.addColorStop(0.8, withAlpha(item.color, 0.44))
  g.addColorStop(0.93, `rgba(255,255,255,0.72)`)
  g.addColorStop(0.99, withAlpha(item.color, 0.12))
...
const drawPrism: Draw = (ctx, item, rand) => {
  ...
  for (let i = 0; i < hues.length; i++) {
    ...
    spike(ctx, a, 0.97 * (0.78 + rand() * 0.3), 0.1, hues[i], 1)
  }
  for (let i = 0; i < 4; i++) spike(ctx, (i / 4) * TAU, 0.32, 0.05, '#ffffff', 0.8)
  core(ctx, 0.17)
}
...
/** A solid four-point sparkle path — the ✨ silhouette, filled rather than
 *  drawn as rays. A filled shape covers area, and area is what registers
 *  against a photograph; a hairline does not, however bright it is. */
function sparklePath(ctx: CanvasRenderingContext2D, r: number, waist = 0.22) {
  ctx.beginPath()
  for (let i = 0; i < 4; i++) {
    const a = -Math.PI / 2 + (i / 4) * TAU
    const next = -Math.PI / 2 + ((i + 1) / 4) * TAU
    const mid = (a + next) / 2
    if (i === 0) ctx.moveTo(Math.cos(a) * r, Math.sin(a) * r)
    else ctx.lineTo(Math.cos(a) * r, Math.sin(a) * r)
    ctx.quadraticCurveTo(
      Math.cos(mid) * waist * r,
      Math.sin(mid) * waist * r,
      Math.cos(next) * r,
      Math.sin(next) * r
    )
  }
...
/** ✨ — a big sparkle with two smaller companions, the shape everyone reads
 *  as "sparkle" at a glance. */
const drawSparkles: Draw = (ctx, item) => {
  bloom(ctx, 0.95, item.color, 0.4)
  const one = (x: number, y: number, r: number, hot: number) => {
    ctx.save()
    ctx.translate(x, y)
    luminousBody(ctx, () => sparklePath(ctx, r), item.color, 0.95)
    core(ctx, r * 0.32, hot)
    ctx.restore()
  }
  one(-0.12, -0.18, 0.66, 1)
  one(0.56, 0.42, 0.3, 0.8)
  one(-0.55, 0.5, 0.22, 0.7)
}

/** Metallic flakes — what is actually in a jar of glitter. */
const drawFoil: Draw = (ctx, item, rand) => {
  bloom(ctx, 1, item.color, 0.16)
  for (let i = 0; i < 11; i++) {
    const a = rand() * TAU
    const d = Math.sqrt(rand()) * 0.82
    const w = 0.1 + rand() * 0.14
    const h = w * (0.5 + rand() * 0.8)
    ctx.save()
    ctx.translate(Math.cos(a) * d, Math.sin(a) * d)
    ctx.rotate(rand() * TAU)
    const g = ctx.createLinearGradient(-w, -h, w, h)
    g.addColorStop(0, withAlpha(item.color, 0.35))
    g.addColorStop(0.45, `rgba(255,255,255,${0.7 + rand() * 0.3})`)
    g.addColorStop(1, withAlpha(item.color, 0.3))
    ctx.fillStyle = g
    ctx.beginPath()
    ctx.moveTo(-w, -h * 0.6)
    ctx.lineTo(w * 0.7, -h)
...
/** A sequin catching the light: a filled disc with a bright rim and one
 *  specular arc. */
const drawSequin: Draw = (ctx, item) => {
  bloom(ctx, 0.95, item.color, 0.32)
  ctx.fillStyle = withAlpha(item.color, 0.6)
  ctx.beginPath()
  ...
  ctx.arc(0, 0, 0.78, 0, TAU)
  ctx.stroke()
  // A soft elliptical highlight where the disc faces the light. An arc with a
  // round cap reads as a stray glyph, not as a reflection.
  ctx.save()
  ctx.translate(-0.26, -0.3)
  ctx.rotate(-0.6)
  ctx.scale(1, 0.5)
  const hi = ctx.createRadialGradient(0, 0, 0, 0, 0, 0.42)
  hi.addColorStop(0, 'rgba(255,255,255,0.9)')
  hi.addColorStop(0.55, 'rgba(255,255,255,0.35)')
...
/** Five petals of light. */
const drawBlossom: Draw = (ctx, item) => {
  bloom(ctx, 0.95, item.color, 0.36)
  for (let i = 0; i < 5; i++) {
    ctx.save()
    ctx.rotate((i / 5) * TAU)
    luminousBody(
      ...
      () => {
        ctx.beginPath()
        ctx.ellipse(0, -0.48, 0.27, 0.44, 0, 0, TAU)
      },
      item.color,
      0.85
    )
...
/** A cut stone: an octagon with facets that catch the light unevenly. */
const drawGem: Draw = (ctx, item) => {
  bloom(ctx, 0.95, item.color, 0.4)
  const oct = (r: number) => {
    ctx.beginPath()
    for (let i = 0; i < 8; i++) {
  ...
  luminousBody(ctx, () => oct(0.88), item.color, 0.95)
  for (let i = 0; i < 8; i++) {
    const a = -Math.PI / 2 + (i / 8) * TAU
    const next = -Math.PI / 2 + ((i + 1) / 8) * TAU
    ctx.fillStyle = `rgba(255,255,255,${i % 2 === 0 ? 0.3 : 0.12})`
    ctx.beginPath()
    ctx.moveTo(0, 0)
    ctx.lineTo(Math.cos(a) * 0.88, Math.sin(a) * 0.88)
    ctx.lineTo(Math.cos(next) * 0.88, Math.sin(next) * 0.88)
    ctx.closePath()
    ctx.fill()
  }
  core(ctx, 0.24, 0.85)
...
/** Foil confetti — small curled ribbons in shifting hues. */
const drawConfetti: Draw = (ctx, item, rand) => {
  bloom(ctx, 1, item.color, 0.14)
  for (let i = 0; i < 12; i++) {
    const a = rand() * TAU
    const d = Math.sqrt(rand()) * 0.8
    const w = 0.09 + rand() * 0.1
    const h = w * (1.4 + rand() * 1.2)
    ctx.save()
    ctx.translate(Math.cos(a) * d, Math.sin(a) * d)
    ctx.rotate(rand() * TAU)
    ctx.fillStyle = shifted(item.color, rand() * 2 - 1, 0.6 + rand() * 0.4)
    ctx.beginPath()
    ctx.moveTo(-w, -h)
    ctx.quadraticCurveTo(w * 1.6, -h * 0.3, -w * 0.4, h)
...
export const SHAPE_DEFS: Record<GlitterShape, ShapeDef> = {
  ...
  halo: { draw: drawHalo },
  sparkles: { draw: drawSparkles },
  foil: { draw: drawFoil },
  sequin: { draw: drawSequin },
  blossom: { draw: drawBlossom },
  gem: { draw: drawGem },
  confetti: { draw: drawConfetti },
